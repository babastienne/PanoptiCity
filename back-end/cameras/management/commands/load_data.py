...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "data_file", help="Mandatory parameter: path to file to import"
        )
        parser.add_argument(
            "--update-cameras",
            "-c",
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            dest="update_building_field",
            help="Force update of buildings (very long task)",
        )
        parser.add_argument(
            "--details",
            "-d",
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for elem in osmium.FileProcessor(
                self.filename, osmium.osm.NODE
                ...
                if "man_made" in elem.tags:
                    if elem.tags["man_made"] == "surveillance":
                        if (
                            update_camera
                            or not Camera.objects.filter(id=elem.id).exists()
                        ):
    ...
    def create_building(self, building_osm):
        try:
            ...
            geometry = Polygon(
                [[x.lat, x.lon] for x in building_osm.get_nodes(resolve_missing=True)],
                srid=4326,
            )  # overpy way
            building = Building.objects.get_or_create(id=building_osm.id, geom=geometry)
            if self.verbose:
                if building[1]:
    ...
    def handle_creation(self, camera_osm):
        ...
        try:
            buffer = str(
                Point([camera_osm.lat, camera_osm.lon], srid=4326).buffer(0.0005).extent
            )
            ...
            for way in result.ways:
                if (
                    ...
                    or not Building.objects.filter(id=way.id).exists()
                ):
                    self.create_building(way)
                else:
                    if self.verbose:
                        self.stdout.write(
                            f"Building #{way.id} already exists. Skipped."
                        )
