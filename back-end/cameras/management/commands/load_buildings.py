...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "building_file", help="Mandatory parameter: path to file to import"
        )
        parser.add_argument(
            "--update",
            "-u",
            ...
            default=False,
            dest="update_field",
            help="Force update of buildings (can take some time)",
        )
        parser.add_argument(
            "--details",
            "-d",
    ...
    def handle(self, *args, **options):
        ...
        try:
            for elem in osmium.FileProcessor(filename).with_filter(
                ...
                if elem.tags["building"] != "roof":
                    if elem.is_way():  # If the object is only a linestring / polygon
                        if not Building.objects.filter(
                            osm_id=elem.id
                        ).exists():  # If it doesn't exists yet
                            imported += 1
                            ...
                            if self.verbose:
                                self.stdout.write(
                                    f"Building #{elem.id} already exists. Skipped."
                                )
                            skipped += 1
                    if elem.is_area():  # If the object is a multipolygon
                        if update:  # If it is an update
                            ...
                            self.stdout.write(
                                f"/!\\ Warning, removing {Building.objects.filter(osm_id=elem.id).count()} buildings (area)"
                            )  # FIXME : Remove when sure it works
                            Building.objects.filter(osm_id=elem.id).delete()
                        for outer in elem.outer_rings():
                            if Building.objects.filter(osm_id=elem.id).count() != len(
                                elem.outer_rings()
                            ):
                                if update:  # If it is an update
                                    # because it can contains multiple objects and we can't distinguish them (same id, different geoms)
                                    # then we remove all buildings with this is and recreate them after
                                    self.stdout.write(
                                        f"|-> Created a new Building with id {elem.id}"
                                    )  # FIXME : Remove when sure it works
                                imported += 1
                                self.create_building(elem.id, outer, total, imported)
                                if imported % 10000 == 0:
                                    self.stdout.write(
                                        f"Imported {imported} elements .."
                                    )
    ...
    def update_building(self, id_building, nodes_building, total, imported):
        try:
            ...
            if self.verbose:
                self.stdout.write(
                    f"{imported}/{total} Building #{id_building} updated."
                )
            return building
        except Exception:
            raise
    ...
    def create_building(self, id_building, nodes_building, total, imported):
        try:
            geometry = Polygon([[x.lon, x.lat] for x in nodes_building], srid=4326)
            building = Building.objects.get_or_create(osm_id=id_building, geom=geometry)
            if self.verbose:
                self.stdout.write(
                    f"{imported}/{total} Building #{id_building} created."
                )
            return building
        except Exception:
            pass
